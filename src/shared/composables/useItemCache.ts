...
class ItemCacheService {
    ...
    try {
      ...
      if (!isPreloaded) {
        ...
        defaultItems.forEach(item => {
          ...
          const exists = shoppingItems.some((cachedItem: CachedItem) =>
            cachedItem.content.toLowerCase() === item.toLowerCase()
          );
  ...
  addItem(type: ListType, content: string) {
    ...
    const exists = items.some((item: CachedItem) => item.content.toLowerCase() === content.toLowerCase());
  ...
  removeItem(type: ListType, content: string) {
    const items = (this.cache.value as ItemCache)[type];
    const index = items.findIndex((item: CachedItem) => item.content.toLowerCase() === content.toLowerCase());
